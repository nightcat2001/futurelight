...
| Parent rights: review | Parent center lists child profiles, learning summaries, rewards, consent records, and recent export request status rows; parent-gated export downloads a machine-readable JSON package. | Implemented local baseline | Add reviewed parent-facing copy and off-box delivery if files are delivered asynchronously in production. |
...
| Data export download | Parent center generates a downloadable JSON export package, records `data_export_generated` in `audit_logs`, and stores a `data_export_requests` status row with expiry metadata. | Implemented local baseline | Add reviewed copy, off-box delivery if needed, support visibility, and production retention/expiry rules. |
