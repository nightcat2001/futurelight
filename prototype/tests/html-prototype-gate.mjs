...
const stateFields = ["default", "loading", "empty", "error", "success", "offline", "disabled", "permissionDenied"];
...
try {
  for (const viewport of contexts) {
    ...
    for (const spec of pages) {
      ...
      if (stateCount >= 8) pass("STATE_CONTROLS", `${viewport.name} ${spec.id} has ${stateCount} state controls`);
      ...
      if (viewport.name === "android-390") {
        await page.screenshot({ path: path.join(screenshotDir, `${spec.id}.png`), fullPage: true });
      }
    }
